...
src/
  ...
  routes/
    ...
    +page.svelte        … 最新開催年（現行）のトップ
    coc/ department/ schedule/ venue/   … 現行サイトの各ページ
    2024/ 2023/         … 過去開催年のアーカイブ（年ごとに独立したページ群）
  lib/
    components/         … 現行サイトの共通コンポーネント
    2024/ 2023/         … 過去開催年専用のコンポーネント・画像
    img/                … 画像
static/                 … そのまま配信される静的アセット
build/                  … ビルド成果物（gitignore 対象）
```

> 過去開催年（`2023` / `2024`）は `routes` と `lib` の両方でディレクトリが分かれている。年をまたいで共通化はせず、その年のディレクトリ内で完結させる方針。
